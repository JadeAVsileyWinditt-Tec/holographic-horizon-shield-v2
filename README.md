...
**Licence:** Source-available. See LICENSE for full terms.  
...
Variance = sum ( (Count of Character / Total Length) ^ 2 )

a = 1.0 + (Variance x 5.0)
...
| Constant                | Value                       | Role                                 |
|-------------------------|-----------------------------|--------------------------------------|
| Orbital period T        | 185.59 / 365.25 ≈ 0.5081 yr | Temporal baseline                    |
| Event-horizon threshold | 9.62 solar masses           | Mass limit that triggers containment |
| Distance constraint     | 1,560.4 light-years         | Token chaotic-drift monitor          |

When calculated mass M exceeds 9.62 solar masses, the request is treated as a Horizon Breach and diverted into the tarpit.
...
### Test endpoints
    ...
    curl -X POST http://localhost:8000/v1/shield/validate -H "Content-Type: application/json" -d "{\"prompt\": \"Your prompt here\"}"
    ---
...
For pilot discussions or custom coefficient sets, see security.md.
---
